			...
			ParaXTokens::transfer(
				...
				Box::new(
					(
						Parent,
						Parachain(100),
						Junction::AccountId32 { network: NetworkId::Kusama, id: BOB.into() },
					)
						.into()
				),
				40,
	...
	ParaA::execute_with(|| {
		...
		assert_ok!(para::OrmlXcm::send_as_sovereign(
			...
			Box::new(VersionedXcm::from(Xcm(vec![
				...
				BuyExecution { fees: assets, weight_limit: Limited(2_000_000_000) },
	...
	ParaA::execute_with(|| {
		...
		assert_err!(
			para::OrmlXcm::send_as_sovereign(
				...
				Box::new(VersionedXcm::from(Xcm(vec![
					WithdrawAsset(assets.clone().into()),
					BuyExecution { fees: assets, weight_limit: Limited(10_000_000) },
					Instruction::Transact {
						origin_type: SovereignAccount,
						require_weight_at_most: 1_000_000_000,
						call: call.encode().into(),
...
fn send_relative_view_sibling_asset_to_non_reserve_sibling() {
	...
	ParaD::execute_with(|| {
		assert_eq!(ParaRelativeTokens::free_balance(CurrencyId::D, &sibling_a_account()), 500);
		assert_eq!(ParaRelativeTokens::free_balance(CurrencyId::D, &sibling_b_account()), 460);
	});
