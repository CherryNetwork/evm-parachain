...
pub mod module {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn transfer_multiassets(
			...
			let fee: &MultiAsset =
				assets.get(fee_item as usize).ok_or(Error::<T>::AssetIndexNonExistent)?;
	...
	impl<T: Config> Pallet<T> {
		fn do_transfer(
			...
			let location: MultiLocation = T::CurrencyIdConvert::convert(currency_id)
		...
		fn do_transfer_multicurrencies(
			...
			let fee_location: MultiLocation =
				T::CurrencyIdConvert::convert(fee_currency_id.clone())
					.ok_or(Error::<T>::NotCrossChainTransferableCurrency)?;
		...
		fn do_transfer_multiassets(
			...
			if fee_reserve != non_fee_reserve {
				...
				let reserve_location =
					non_fee_reserve.clone().ok_or(Error::<T>::AssetHasNoReserve)?;
				let min_xcm_fee =
					T::MinXcmFee::get(&reserve_location).ok_or(Error::<T>::MinXcmFeeNotDefined)?;
		...
		fn execute_and_send_reserve_kind_xcm(
			...
			let weight =
				T::Weigher::weight(&mut msg).map_err(|()| Error::<T>::UnweighableMessage)?;
		...
		fn transfer_to_non_reserve(
			...
			if reserve == MultiLocation::parent() {
				match dest {
					MultiLocation { parents, interior: X1(Parachain(id)) } if parents == 1 => {
		...
		fn deposit_asset(recipient: MultiLocation, max_assets: u32) -> Instruction<()> {
			DepositAsset { assets: All.into(), max_assets, beneficiary: recipient }
		}
		...
		fn ensure_valid_dest(
			dest: &MultiLocation,
		) -> Result<(MultiLocation, MultiLocation), DispatchError> {
	...
	impl<T: Config> Pallet<T> {
		...
		fn weight_of_transfer_multicurrencies(
			...
			Self::weight_of_transfer_multiassets(
				&VersionedMultiAssets::from(MultiAssets::from(assets)),
				fee_item,
				dest,
			)
		}
		...
		fn weight_of_transfer_multiassets(
			...
			if let (Ok(assets), Ok(dest)) = (assets, dest) {
				...
				if let Ok((transfer_kind, dest, _, reserve)) =
					Self::transfer_kind(reserve_location, &dest)
				{
					let mut msg = match transfer_kind {
						SelfReserveAsset =>
							Xcm(vec![TransferReserveAsset { assets, dest, xcm: Xcm(vec![]) }]),
						ToReserve | ToNonReserve => Xcm(vec![
...
fn half(asset: &MultiAsset) -> MultiAsset {
	...
	MultiAsset { fun: Fungible(half_amount), id: asset.id.clone() }
}
...
fn subtract_fee(asset: &MultiAsset, amount: u128) -> MultiAsset {
	...
	MultiAsset { fun: Fungible(final_amount), id: asset.id.clone() }
}
