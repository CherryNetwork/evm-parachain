...
impl Convert<CurrencyId, Option<MultiLocation>> for CurrencyIdConvert {
	fn convert(id: CurrencyId) -> Option<MultiLocation> {
		match id {
			...
			CurrencyId::A1 =>
				Some((Parent, Parachain(1), GeneralKey(b"A1".to_vec().try_into().unwrap())).into()),
			CurrencyId::B =>
				Some((Parent, Parachain(2), GeneralKey(b"B".to_vec().try_into().unwrap())).into()),
			CurrencyId::B1 =>
				Some((Parent, Parachain(2), GeneralKey(b"B1".to_vec().try_into().unwrap())).into()),
			CurrencyId::B2 =>
				Some((Parent, Parachain(2), GeneralKey(b"B2".to_vec().try_into().unwrap())).into()),
			CurrencyId::C =>
				Some((Parent, Parachain(3), GeneralKey(b"C".to_vec().try_into().unwrap())).into()),
			CurrencyId::D =>
				Some((Parent, Parachain(4), GeneralKey(b"D".to_vec().try_into().unwrap())).into()),
		}
	}
}
...
pub fn para_ext(para_id: u32) -> TestExternalities {
	...
	orml_tokens::GenesisConfig::<Runtime> { balances: vec![(ALICE, CurrencyId::R, 1_000)] }
