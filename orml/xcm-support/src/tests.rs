...
impl Convert<MultiLocation, Option<TestCurrencyId>> for CurrencyIdConvert {
	fn convert(l: MultiLocation) -> Option<TestCurrencyId> {
		...
		if l == MultiLocation::sibling_parachain_general_key(
			1,
...
fn is_native_concrete_matches_native_currencies() {
	assert_eq!(MatchesCurrencyId::matches_fungible(&MultiAsset::parent_asset(100)), Some(100),);
...
fn is_native_concrete_does_not_matches_non_native_currencies() {
	assert!(<MatchesCurrencyId as MatchesFungible<u128>>::matches_fungible(
		&MultiAsset::sibling_parachain_asset(2, b"TokenC".to_vec().try_into().unwrap(), 100)
	)
	.is_none());
	assert!(<MatchesCurrencyId as MatchesFungible<u128>>::matches_fungible(
		&MultiAsset::sibling_parachain_asset(1, b"TokenB".to_vec().try_into().unwrap(), 100)
	)
	.is_none());
	assert!(<MatchesCurrencyId as MatchesFungible<u128>>::matches_fungible(&MultiAsset {
		fun: Fungible(100),
		id: Concrete(MultiLocation::new(1, X1(GeneralKey(b"TokenB".to_vec().try_into().unwrap())))),
	})
	.is_none());
}
...
fn multi_native_asset() {
	assert!(MultiNativeAsset::<AbsoluteReserveProvider>::filter_asset_location(
		&MultiAsset { fun: Fungible(10), id: Concrete(MultiLocation::parent()) },
		&Parent.into()
	));
	assert!(MultiNativeAsset::<AbsoluteReserveProvider>::filter_asset_location(
